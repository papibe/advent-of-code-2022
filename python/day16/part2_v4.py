...
def solve(
    ...
    minute: int = 0
    queue: Deque = deque()
    # queue.append((minute, "AA", 0, opened_valves_setup))
    # print(opened_valves_setup)
    ...
    while queue:
        ...
        if valves[valve].flow > 0 and valve not in opened:
            ...
            queue.append((minute + 1, valve, current_pressure, new_history, new_opened))
            state.add((minute + 1, valve, current_pressure))
