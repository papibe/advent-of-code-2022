...
def solve(blue_print, work_force, current_resources, working_minutes) -> int:
    ...
    visited: Set = set()
    key = tuple([0, *work_force.values(), *current_resources.values()])
    visited.add(key)
    ...
    while heap:
        ...
        key = tuple([minute + 1, *work_force.values(), *new_current_resources.values()])
        if key in visited:
            # print("save", key)
            saves += 1
            continue
        ...
        for resource, cost in reversed(blue_print.items()):
            for element, quantity in cost.items():
                ...
                key = tuple(
                    [minute + 1, *new_work_force.values(), *new_resources.values()]
                )
