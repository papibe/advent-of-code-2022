...
func solve(
	...
	return dfs(max_minutes, aa_index, opened_valves|(1<<aa_index))
}
...
func solution(filename string) int {
	...
	max_team_pressure := 0
	times := (1 << len(new_flows)) - 1
	for bitmask := range times / 2 {
...
func main() {
	fmt.Println(solution("./example.txt")) // 1707
	fmt.Println(solution("./input.txt"))   // 2520
}
