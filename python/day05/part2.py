...
def solve(stacks: Stack, instructions: List[Instruction]) -> str:
    for instr in instructions:
        stack_hight: int = len(stacks[instr.source])
        crates_to_move: List[str] = stacks[instr.source][-instr.quantity :]
        stacks[instr.source] = stacks[instr.source][: stack_hight - instr.quantity]
        stacks[instr.destination].extend(crates_to_move)
...
if __name__ == "__main__":
    print(solution("./example.txt"))  # "MCD"
    print(solution("./input.txt"))  # "LVZPSTTCZ"
