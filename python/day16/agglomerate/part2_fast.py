...
def solve(
    ...
    def sol(minutes: int, valve: str, opened: int):
        ...
        possible_steps = {
            flows[neighbor] * (minutes - distance - 1): neighbor
            for neighbor, distance in enumerate(am[valve])
            if (opened & (1 << neighbor)) == 0
        }
        if not possible_steps:
            return 0
        # print(possible_steps)
        max_next_flow = max(possible_steps.keys())
        if max_next_flow <= 0:
            return 0
        next_valve = possible_steps[max_next_flow]

        # print(possible_steps)
        # print(max_next_flow)
        # print()

        next_valve_bit = 1 << next_valve
        if opened & next_valve_bit == 0:  # not opened
            max_pressure = max_next_flow
            + sol(minutes - am[valve][next_valve] - 1, next_valve, opened | next_valve_bit)
