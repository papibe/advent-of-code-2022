...
class Board:
    def __init__(
        ...
        self.start: Tuple[int, int] = (0, layout[0].count("0") * side)
    ...
    def move(self, current: Tuple[int, int], direction: Direction) -> Tuple:
        ...
        if (
            ...
            # print(face_row, face_col)
            current_face = self.layout[face_row][face_col]

            if current_face not in self.stiches or direction.value not in self.stiches[current_face]:
                print(f"\n\nIncomplete stiches {current_face} {direction.value}\n\n")
                raise KeyError
            ...
            next_relative_row = (
                current_relative_row * row_func[0] + current_relative_col * row_func[1] + row_func[2]
            ) % self.side
            next_relative_col = (
                current_relative_row * col_func[0] + current_relative_col * col_func[1] + col_func[2]
            ) % self.side
...
def solve(board_map: Board, path: str) -> int:
    ...
    print(
        f"{current_row = }, {current_col = }, {direction_value[facing] = }"
    )
...
if __name__ == "__main__":
    ...
    stitches = {
        "b": {">": ("o", Direction.down, (0, 0, 0), (1, 0, -fs))},  # f((c, r), (c, r)) ??
        "y": {"v": ("g", Direction.up, (0, 0, fs), (0, -1, fs))},   #row -> fs, invert cols
        "r": {"^": ("w", Direction.left, (0, 1, 0), (0, 0, 0))},   #col -> row, col -> 0
    }

    print(solution("./example.txt", 4, example_layout, stitches))   # it should be 5031
    ...
    stitches = {
        "w": {
            "^": ("g", Direction.right, (0, 1, 0), (0, 0, 0)), #
            "<": ("r", Direction.right, (-1, 0, fs), (0, 0, 0)), #
        },  
        "g": {
            "<": ("w", Direction.down, (0, 0, 0), (1, 0, 0)), #
            "v": ("o", Direction.down, (0, 0, 0), (0, 1, 0)), #
            ">": ("y", Direction.up, (0, 0, fs), (1, 0, 0)), #
        },
        "r": {
            "^": ("b", Direction.right, (0, 1, 0), (0, 0, 0)), #
            "<": ("w", Direction.right, (-1, 0, fs), (0, 0, 0)), #
        },
        "o": {
            "^": ("g", Direction.up, (0, 0, fs), (0, 1, 0)), #
            "v": ("b", Direction.left, (0, 1, 0), (0, 0, fs)), #
            ">": ("y", Direction.left, (-1, 0, fs), (0, 0, fs)), #
        },
        "b": {
            "<": ("r", Direction.down, (0, 0, 0), (0, 1, 0)), #
            ">": ("o", Direction.up, (0, 0, fs), (1, 0, 0)), #
        },
        "y": {
            "v": ("g", Direction.left, (0, 1, 0), (0, 0, fs)), #
            ">": ("o", Direction.left, (-1, 0, fs), (0, 0, fs)), #
        }

    }
    # print(solution("./input.txt", 50, input_layout, stitches))  # not 19451, 58214, 179079
