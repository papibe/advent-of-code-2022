...
def solve2(blue_print: Dict, work_force: Dict, working_minutes: int) -> int:
    ...
    queue: Deque = deque()
    queue.append((0, work_force, collections))
    visited: Set = set()
...
def solve3(blue_print, work_force, current_resources, state, reminding_minutes) -> int:

    # print(reminding_minutes, work_force, current_resources)

    state_key = tuple(
        [reminding_minutes, *current_resources.values(), *work_force.values()]
    )
    # print(state_key)

    # return 0

    if reminding_minutes == 0:
        state[state_key] = current_resources["geode"]
        return current_resources["geode"]

    if state_key in state:
        # print("-" * 50)
        # print("save: ", state_key)
        # print("-" * 50)
    ...
    # max_gathering: int = current_resources["geode"] + work_force["geode"] * reminding_minutes
    ...
    for resource, cost in blue_print.items():
        for element, quantity in cost.items():
            ...
            # print(f"I can buy {resource} robot")
            ...
            max_gathering = max(max_gathering, current_gathering)
    ...
    state[state_key] = max_gathering
    return max_gathering
