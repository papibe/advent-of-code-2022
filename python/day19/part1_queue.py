...
def produce(
    ...
    queue: Deque[Tuple[int, Robots, Resources]] = deque(
        [(working_minutes, work_force, current_resources)]
    )
    ...
    while queue:
        ...
        normalize_and_shrink(max_spend, work_force, current_resources, minute)

        # key = get_key(minute, max_spend, work_force, current_resources)
        key: Tuple[int, ...] = (
            minute,
            *work_force.values(),
            *current_resources.values(),
        )
        ...
        # not building ---------------------------------------------------------
        new_current_resources: Resources = current_resources.copy()

        # produce
        for robot in work_force:
            new_current_resources[robot] += work_force[robot]

        new_resources_backup: Resources = new_current_resources.copy()

        queue.append((minute - 1, work_force.copy(), new_current_resources))

        # Check if we can build ------------------------------------------------
        for resource, cost in blue_print.items():
            if work_force[resource] >= max_spend[resource]:
                continue
            for element, quantity in cost.items():
                ...
                # built
                new_force: Robots = work_force.copy()
                new_force[resource] += 1

                queue.append((minute - 1, new_force, new_resources))
