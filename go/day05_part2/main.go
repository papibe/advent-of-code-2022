...
func solve(stacks Stack, instructions []Instruction) string {
	for _, instr := range instructions {
		stack_hight := len(stacks[instr.source])

		// pop creates from source
		creates := stacks[instr.source][len(stacks[instr.source])-instr.quantity:]
		stacks[instr.source] = stacks[instr.source][:stack_hight-instr.quantity]

		// add create to destination
		stacks[instr.destination] = append(stacks[instr.destination], creates...)
	}
...
func main() {
	fmt.Println(solution("./example.txt")) // "MCD"
	fmt.Println(solution("./input.txt"))   // "LVZPSTTCZ"
}
